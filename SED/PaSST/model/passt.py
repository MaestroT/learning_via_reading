...
class PaSST(nn.Module):
    ...
    '''
    The forward_features function is defined in the vision_transformer.py file 
    in the timm module. It is a helper function that is used in the ViT model 
    to apply the layers of the model to the input tensor. The forward_features 
    function takes an input tensor and a model as input and returns the output 
    tensor of the model. The function first applies the patch embedding layer 
    to the input tensor to convert it into a sequence of patches. It then applies 
    the transformer encoder to the sequence of patches to obtain the output tensor. 
    Finally, it applies the MLP head to the output tensor to obtain the final output 
    of the model. The forward_features function is used in the forward method of the 
    VisionTransformer class to process the input tensor.
    '''
