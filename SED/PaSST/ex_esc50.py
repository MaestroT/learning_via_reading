...
class M(Ba3lModule):
    ...
    def mel_forward(self, x):
        old_shape = x.size() #  [B C T]
        x = x.reshape(-1, old_shape[2]) # [B*C T]
        x = self.mel(x)
        x = x.reshape(old_shape[0], old_shape[1], x.shape[1], x.shape[2]) # [B C F T], (batch_size, num_channels, num_mel_bins, num_frames)
        if self.dyn_norm:
    ...
    def training_step(self, batch, batch_idx):
        # REQUIRED
        x, f, y = batch # audio_signal, file_name, target_label
        if self.mel:
